...
- [ ] Task 1: Create `PushSubscription` SQLAlchemy model (AC: 1)
  - [ ] Create `backend/app/models/push_subscription.py`
  - [ ] Model fields: `id` (int PK autoincrement), `endpoint` (Text, not null, unique), `p256dh` (Text, not null), `auth` (Text, not null), `created_at` (DateTime timezone=True, server_default=func.now())
  - [ ] Import `Base` from `app.models.base`
  - [ ] Register the model in `backend/alembic/env.py` — add `from app.models import push_subscription  # noqa: F401` alongside the existing `task` import

- [ ] Task 2: Generate and apply Alembic migration (AC: 1)
  - [ ] `cd backend && uv run alembic revision --autogenerate -m "add push_subscriptions table"`
  - [ ] Review the generated migration file in `alembic/versions/` — ensure `push_subscriptions` table has all required columns including `unique=True` on `endpoint`
  - [ ] `cd backend && uv run alembic upgrade head` — apply migration
  - [ ] Verify: `uv run python -c "import sqlite3; c=sqlite3.connect('simple-todo.db'); print(c.execute(\"SELECT name FROM sqlite_master WHERE type='table'\").fetchall())"` — `push_subscriptions` must appear

- [ ] Task 3: Create Pydantic schemas for push (AC: 2)
  - [ ] Create `backend/app/schemas/push.py`
  - [ ] `PushSubscribeRequest(BaseModel)`: fields `endpoint: str`, `p256dh: str`, `auth: str`
  - [ ] `PushSubscriptionRead(BaseModel)`: fields `id: int`, `endpoint: str`, `p256dh: str`, `auth: str`, `created_at: datetime` — with `model_config = ConfigDict(from_attributes=True)`

- [ ] Task 4: Create `push_service.py` — web push dispatch (AC: 3, 4)
  - [ ] Create `backend/app/services/push_service.py`
  - [ ] Implement `def send_web_push(subscription, payload: dict) -> None:`
  - [ ] Use `pywebpush.webpush()` (sync, not `webpush_async`) — it is called from scheduler jobs which run in APScheduler's thread pool, not the async event loop
  - [ ] `subscription_info` format (required by pywebpush):
    ```python
    {
    ...
    ```
  - [ ] `vapid_private_key`: pass `settings.vapid_private_key` (string from env)
  - [ ] `vapid_claims`: `{"sub": f"mailto:{settings.vapid_claim_email}"}`
  - [ ] `data`: `json.dumps(payload)` (serialize dict to string)
  - [ ] Wrap the `webpush()` call in `try/except WebPushException as e:` — log the error (`logger.error("Push delivery failed: %s", e)`) and return without re-raising (AC: 4)
  - [ ] Also catch generic `Exception` as a fallback — log and swallow (AC: 4)
  - [ ] Use `logging.getLogger(__name__)` for the module logger
  - [ ] Guard: if `settings.vapid_private_key` is empty/blank, log a warning and return immediately — do not attempt to call pywebpush with missing keys

- [ ] Task 5: Create `push.py` router — subscribe endpoint (AC: 2)
  - [ ] Create `backend/app/routers/push.py`
  - [ ] `router = APIRouter(prefix="/api/v1/push", tags=["push"])`
  - [ ] Implement `POST /subscribe`: upsert logic — check if `endpoint` already exists in `push_subscriptions`; if yes, update `p256dh` and `auth` and return `200`; if no, insert new row and return `201`
  - [ ] Place DB logic in the router (this is simple enough — no separate service file needed for upsert)
  - [ ] Request body: `PushSubscribeRequest`; Response: `PushSubscriptionRead`
  - [ ] Use `Response` class to set dynamic status code (201 vs 200):
    ```python
    @router.post("/subscribe", response_model=PushSubscriptionRead)
    async def subscribe(body: PushSubscribeRequest, response: Response, db: AsyncSession = Depends(get_db)):
        ...
        response.status_code = 201  # or 200
    ```
  - [ ] Register the router in `backend/app/main.py`: `from app.routers import push as push_router` then `app.include_router(push_router.router)`

- [ ] Task 6: Write backend tests (AC: 1–4)
  - [ ] Create `backend/tests/test_push.py`
  - [ ] Test: `POST /api/v1/push/subscribe` with new endpoint → 201, row in DB
  - [ ] Test: `POST /api/v1/push/subscribe` with same endpoint → 200, row updated (p256dh/auth refreshed)
  - [ ] Test: `push_service.send_web_push()` with `WebPushException` → logs error, does NOT raise (use `unittest.mock.patch` to mock `pywebpush.webpush`)
  - [ ] Test: `push_service.send_web_push()` with empty `vapid_private_key` → returns without calling pywebpush
  - [ ] Use the existing `client` and `db_session` fixtures from `conftest.py`

- [ ] Task 7: Verify (AC: 1–4)
  - [ ] `cd backend && uv run uvicorn app.main:app --reload` — app starts, `/docs` shows push endpoint
...
- `backend/alembic/versions/<hash>_add_push_subscriptions_table.py` (NEW — auto-generated)
